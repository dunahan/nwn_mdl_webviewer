...
function parseNode(lines, start) {
  ...
  while (i < lines.length) {
    ...
    if      (k === 'parent')      node.parent = t[1] || 'NULL';
      ...
      // skin-Node: pro Vertex eine Zeile mit "BoneName Gewicht BoneName Gewicht ..."
      // Wir summieren die Gewichte pro Bone über alle Vertices und merken uns
      // den Bone mit dem höchsten Gesamtgewicht (= "dominant bone").
      // Dieser wird später in buildScene genutzt, um das Skin-Mesh als Kind
      // des dominanten Bones einzuhängen, damit es der Animation folgt.
      const count = parseInt(t[1]) || 0;
      const boneWeightSum = {};
      for (let j = 0; j < count; j++) {
        i++;
        ...
        for (let w = 0; w + 1 < wt.length; w += 2) {
          ...
          boneWeightSum[bone] = (boneWeightSum[bone] || 0) + weight;
        }
      }
      // Dominanten Bone bestimmen
      let dominantBone = null, maxWeight = -1;
      for (const [bone, total] of Object.entries(boneWeightSum)) {
        if (total > maxWeight) { maxWeight = total; dominantBone = bone; }
      }
      node.dominantBone = dominantBone;
    }
    i++;
  }
  return { node, next: i };
