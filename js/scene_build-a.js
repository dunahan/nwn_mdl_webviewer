...
function buildScene(model) {
  ...
  // ── Skin-Nodes an dominanten Bone hängen (approximatives Skinning) ──────
  // Da kein GPU-Skinning implementiert ist, folgt die Flügelhaut den Bones
  // nicht automatisch. Lösung: Das Skin-Mesh wird als Kind des Bones mit dem
  // höchsten Gesamtgewicht eingehängt. Three.js .attach() berechnet die neue
  // lokale Transform so, dass die Welt-Position erhalten bleibt.
  // Wichtig: updateMatrixWorld() VOR attach() aufrufen — Three.js berechnet
  // Matrizen lazy; ohne diesen Schritt arbeitet attach() mit veralteten Werten.
  modelGroup.updateMatrixWorld(true);
  for (const node of model.nodes) {
    if (node.type !== 'skin' || !node.dominantBone) continue;
    const skinObj = objects[node.name];
    const boneObj = objects[node.dominantBone];
    if (!skinObj || !boneObj) continue;
    boneObj.attach(skinObj);   // re-parent, Welt-Transform bleibt erhalten
  }

  // Geometrie-Pose erst NACH dem Re-Parent speichern, damit resetToPose()
  // die korrekten lokalen Positionen (relativ zum neuen Parent) kennt.
  saveGeometryPose();
  buildAnimUI(model);
}
